 ...
 */
		...
		function getDate(f_info) {
			...
			$('#f_date ul').remove();
			$('#f_date ul li').remove();
			$('#f_date ul li a').remove();
			
			$('#f_time ul').remove();
			$('#f_time ul li').remove();
			$('#f_time ul li a').remove();
			
			$('#f_spot p').remove(); 
			
			$("#f_reserve p").remove();
			$("#f_reserve select").remove();
			$("#f_reserve input").remove();
			...
			$.getJSON(f_info, function(data) {
				...
				//날짜 목록
				$.each(date, function(index, entry) {

					html = '<ul><li><a href="#" class="date" value="' +entry+ '">'+ entry + '</a></li></ul>';
			...
			$.getJSON(f_info, function(data) {
				...
				$.each(runningTime, function(index, entry) {
					...
					html = '<ul><li><a href="#" class="time" value="'+entry.t_id+'">'
							+ entry.t_start + '~' + entry.t_end + '</a></li></ul>';
		...
		/*		var success = data.success;
				var new_spot= data.new_spotList;
				
				$.each(new_spot, function(index, entry) {
